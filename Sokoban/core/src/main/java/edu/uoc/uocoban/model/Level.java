...
public class Level {
    ...
    /*public void updateBoard() {
        try {
            ...
            for (int y = 0; y < getHeight(); y++) {
                for (int x = 0; x < getWidth(); x++) {
                    str.append(getMapItem(x, y).getSprite().getSymbol());
                }
    ...
    public boolean hasWon() {
        ...
        for (MapItem item: getMapItemList()) {
            if (item instanceof Destination && ((Destination) item).getBox() == null) {
                return false;
            }
        }
        return true;
    ...
    private boolean isDeadlocked(Box box) {
        ...
        int counter = 0;
        for (Direction dir: Direction.values()) { // Cornered box
            if (getMapItem(box.getPosition().getX()+dir.getX(), box.getPosition().getY()+dir.getY()).getSprite() == Sprite.WALL){
                counter ++;
                if (counter == 2){
                    return true;
                }
        ...
        Position posBox = new Position(0,0);
        for (Direction dir: Direction.values()) { // 2 boxes
            if (getMapItem(box.getPosition().getX()+dir.getX(), box.getPosition().getY()+dir.getY()).getSprite() == Sprite.BIG_BOX ||
                    getMapItem(box.getPosition().getX()+dir.getX(), box.getPosition().getY()+dir.getY()).getSprite() == Sprite.SMALL_BOX){
                counter ++;
                posBox.setX(box.getPosition().getX()+dir.getX());
                posBox.setY(box.getPosition().getY()+dir.getY());
            }
            if (counter >= 1 && getMapItem(posBox.getX()+dir.getX(), posBox.getY()+dir.getY()) != null) {
                if (getMapItem(box.getPosition().getX()+dir.getX(), box.getPosition().getY()+dir.getY()).getSprite() == Sprite.WALL &&
                getMapItem(posBox.getX()+dir.getX(), posBox.getY()+dir.getY()).getSprite() == Sprite.WALL) {
                    return true;
                }
            }
        }
    ...
    private boolean isPathableOrMovable(int x, int y) {
        //TODO
        return getMapItem(x, y).isPathable() || getMapItem(x, y).getSprite() == Sprite.PLAYER ||
                (getMapItem(x, y).getSprite() == Sprite.BIG_BOX && !((BigBox) getMapItem(x, y)).isDelivered()) ||
                (getMapItem(x, y).getSprite() == Sprite.SMALL_BOX && !((SmallBox) getMapItem(x, y)).isDelivered());
    }
