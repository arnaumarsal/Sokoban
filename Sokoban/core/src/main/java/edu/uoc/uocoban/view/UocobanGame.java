...
public class UocobanGame extends Game {
    ...
    public void loadImages() {
        ...
        wallAsset = new AssetDescriptor<>(WALL.getImageSrc(), Texture.class);
        pathAsset = new AssetDescriptor<>(PATH.getImageSrc(), Texture.class);
        playerAsset = new AssetDescriptor<>(PLAYER.getImageSrc(), Texture.class);
    ...
    AssetDescriptor<Texture> getAssertBySprite(Sprite sprite) {
        ...
        if (sprite == Sprite.WALL) {
            return wallAsset;
        } else if (sprite == Sprite.PATH) {
            return pathAsset;
        } else if (sprite == Sprite.PLAYER) {
            return playerAsset;
        } else if (sprite == Sprite.BIG_BOX) {
            return bigCrateAsset;
        } else if (sprite == Sprite.BIG_BOX_DESTINATION) {
            return bigCrateDestinationAsset;
        } else if (sprite == Sprite.BIG_BOX_ON_DESTINATION) {
            return bigCrateOnDestinationAsset;
        } else if (sprite == Sprite.SMALL_BOX) {
            return smallCrateAsset;
        } else if (sprite == Sprite.SMALL_BOX_DESTINATION) {
            return smallCrateDestinationAsset;
        } else if (sprite == Sprite.SMALL_BOX_ON_DESTINATION) {
            return smallCrateOnDestinationAssset;
        }
        return null;
    }
